#!/usr/bin/env python3
"""Train behavior cloning from cached HM3D RGB encoder features."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train HM3D imitation policy from cached RGB features.")
    ...
    parser.add_argument("--feature_key", type=str, default="rgb_features")
...
def main():
    ...
    print(
        f"[INFO] HM3D feature IL dataset windows={len(dataset)} files={len(dataset.files)} "
        f"actions={dataset.num_actions} feature_dim={dataset.feature_dim}"
    )
    print(f"[INFO] Using device: {device}")
    print("[INFO] Navigation config:")
    print(json.dumps(navigation_config, indent=2))

    if int(dataset.feature_dim) != int(navigation_config["rgb_dim"]):
        raise ValueError(
            f"Cached feature dim {dataset.feature_dim} does not match navigation rgb_dim "
            f"{navigation_config['rgb_dim']}"
        )
