...
LOG_FILE="${HM3D_IL_LOG_FILE:-$ROOT/train_log/hm3d_il_train_features_$TS.log}"
PID_FILE="${HM3D_IL_PID_FILE:-$ROOT/train_log/hm3d_il_train_features_$TS.pid}"
...
echo "[INFO] Starting HM3D feature IL training"
...
echo "[INFO] IL_NUM_WORKERS: ${IL_NUM_WORKERS:-4}"
echo "[INFO] IL_FREEZE_ENCODER: ${IL_FREEZE_ENCODER:-1}"
echo "[INFO] IL_USE_TRANSFORMER: ${IL_USE_TRANSFORMER:-0}"

FREEZE_ENCODER_ARGS=()
if [[ "${IL_FREEZE_ENCODER:-1}" == "1" || "${IL_FREEZE_ENCODER:-1}" == "true" || "${IL_FREEZE_ENCODER:-1}" == "TRUE" ]]; then
  FREEZE_ENCODER_ARGS=(--freeze_encoder)
fi

TRANSFORMER_ARGS=()
if [[ "${IL_USE_TRANSFORMER:-0}" == "1" || "${IL_USE_TRANSFORMER:-0}" == "true" || "${IL_USE_TRANSFORMER:-0}" == "TRUE" ]]; then
  TRANSFORMER_ARGS=(--use_transformer)
fi

exec /root/miniconda3/envs/habitat/bin/python "$ROOT/ImitationLearning/train_hm3d_il_features.py" \
  --conf_path "$ROOT/config" \
  --data_dir "$FEATURE_DIR" \
  --save_dir "$SAVE_DIR" \
  --epochs "${IL_EPOCHS:-30}" \
  --batch_size "${IL_BATCH_SIZE:-128}" \
  --seq_len "${IL_SEQ_LEN:-16}" \
  --lr "${IL_LR:-0.0001}" \
  --train_epoch_samples "${IL_TRAIN_EPOCH_SAMPLES:-80000}" \
  --val_epoch_samples "${IL_VAL_EPOCH_SAMPLES:-16000}" \
